...
import {debounceTime, filter, flatMap, map, switchMap, take, tap} from "rxjs/operators";
...
export class EntityListComponent implements OnInit {
  ...
  private addListeners() {
    ...
    let getList$ = (query) => {
      let update$;
      if(live) {
        update$ = this.broadcast.on('update-page-data').pipe(
          debounceTime(10000),
          switchMap(() => {
        ...
        update$ = this.broadcast.on('update-page-data').pipe(
          ...
          flatMap((currentData) => {
            ...
            return this.getListApi(query).pipe(
              map((updatedPageData: IPageData) => {
    ...
    let sub = this.getQueryWithCallBack().pipe(
      switchMap(query => getList$(query).pipe(
        tap(() => {
    ...
    let sub1 = this.getListDateQuery().pipe(
      tap((query) => {
  ...
  updateSummary(query) {
    let updateSummary$ = this.broadcast.on('update-page-data').pipe(debounceTime(10000));
