...
export const addDataFactory = (mapEntity: MapEntities<any>) => {

  return {
    ...mapEntity,
    dataSub: null,
    setData$(data$: Observable<IUserData | null>, dataType: 'list' | 'item' = 'list') {
      if (this.dataSub) {
        this.dataSub.unsubscribe();
      }
      this._initDataObserver(data$, dataType)
    },
    _initData$(data$: Observable<IUserData | null>, dataType: 'list' | 'item' = 'list') {
    ...
    _initDataObserver(data$: Observable<IUserData | null>, dataType: 'list' | 'item' = 'list') {
      // Observable.of(3).let(
      //   filter((x: number) => !!x)
      // )
      // data$
