...
import { IUser, Page } from "ht-models";
...
export class UsersAnalytics extends EntityListClient {
  ...
  api$: (query) => Observable<Page<IUser>>;
  name = "users analytics list";
  defaultQuery = { ordering: "-last_heartbeat_at",status: "completed" };
  dataSub: Subscription;
  // allowedQueryKeys = null;
  // active$ = store.select(fromRoot.getUsersAnalyticsIsActive);
  data$: Observable<Page<IUser>>;
  dataArray$: Observable<IUser[] | null>;
  ...
  constructor({ dateRange, store, dateParam, api }: IPageClientConfig<HtUsersApi>) {
    super();
    this.api$ = (query): Observable<Page<IUser>> =>
      api.index(query);
    ...
    this.data$ = this.store.select(fromRoot.getUsersIndexPage);
  ...
  setData(data) {
    this.store.dispatch(new fromUsersDispatcher.SetUsersIndexPage(data));
  }
