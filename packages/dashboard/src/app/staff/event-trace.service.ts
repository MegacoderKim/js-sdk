...
export class EventTraceService {
  ...
  getPosition(event) {
    return L.latLng(event.location.geojson.coordinates[1], event.location.geojson.coordinates[0])
  }
  ...
  renderPolyline(encodedPolyline: string, type: string) {
    ...
    if(debugPolyline) {
      ...
      SetEncodedPath(polyline, encodedPolyline);
      polyline.addTo(this.mapService.map);
      this.debugPolylines[type] = polyline;
      // this.setBounds();
    }
  }
    ...
    return _.map(events, (event: ISdkEvent) => {
      if(event.type == 'stop.ended') {
        ...
        if(stopStart) {
          ...
          let start = L.latLng(startCord[1], startCord[0]);
          let end = L.latLng(endCord[1], endCord[0]);
