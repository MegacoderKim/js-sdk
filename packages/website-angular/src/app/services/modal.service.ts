...
export class ModalService {
  ...
  open(modalId: string): void {
    ...
    if (modalId === 'signupModal') {
      this.externalAnalyticsService.logSegmentEvent( "signup modal open", "interaction", "signup" )
    }
    if (modal) {
      modal.isOpen = true;
    }
  ...
  close(modalId: string, checkBlocking = false): void {
    const modal = this.findModal(modalId);
    if (modalId === 'signupModal') {
      this.externalAnalyticsService.logSegmentEvent( "signup modal close", "interaction", "signup" )
    }
